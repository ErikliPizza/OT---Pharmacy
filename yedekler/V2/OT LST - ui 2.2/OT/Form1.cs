...
namespace OT
{
	public partial class Form1 : MetroSetForm
	{
        ...
        {
            if (tabControl.SelectedTab == tabControl.TabPages["tabPageFirst"])
            {
                getProducts();

            }
            else if (tabControl.SelectedTab == tabControl.TabPages["tabPageSecond"])
            {
                scanBool = true;
                deleteBool = false;
                sellBool = false;
            ...
            else if (tabControl.SelectedTab == tabControl.TabPages["tabPageThird"])
            {
                getLogs();
            }
            ...
            if (dreader.HasRows)
            {
                ...
                foreach (DataRowView rowView in dv)
                {
                    if (rowView["BARKOD"].ToString() == b)
                    {
                        ...
                        rowView["TOPLAM"] = ((isS+1)*isF).ToString();
                        _conn.Close();
                        sellBool = true;
                        deleteBool = true;
                        scanBool = true;
        ...
        private void sellProduct()
        {
            ...
            foreach (DataRowView rowView in dv)
            {
                ...
                _cmd.Parameters.AddWithValue("@istotal", float.Parse(rowView["FIYAT"].ToString())*Convert.ToInt32(rowView["ADET"].ToString()));
                try
                {
                    _conn.Open();
                    _cmd.ExecuteNonQuery();
        ...
        {
            if (string.IsNullOrWhiteSpace(datagridMain.CurrentRow.Cells["ADET"].Value.ToString())|| datagridMain.CurrentRow.Cells["ADET"].Value.ToString() == "0") datagridMain.CurrentRow.Cells["ADET"].Value = "1";

            int a = Convert.ToInt32(datagridMain.CurrentRow.Cells["STOK"].Value);
            float s = Convert.ToInt32(datagridMain.CurrentRow.Cells["ADET"].Value);
            float isF;
            if (s > a)
            {
        ...
        {
            if (e.KeyCode == Keys.Enter && tabControl.SelectedTab == tabControl.TabPages["tabPageSecond"])
            {
                if (scanBool == true)
                {
                    scanBool = false;
            ...
            if (e.KeyCode == Keys.Space && tabControl.SelectedTab == tabControl.TabPages["tabPageSecond"])
            {
                if (sellBool == true)
                {
                    scanBool = false;
            ...
            if (e.KeyCode == Keys.Delete || e.KeyCode == Keys.Back && tabControl.SelectedTab == tabControl.TabPages["tabPageSecond"])
            {
                if (deleteBool == true)
                {
                    if (editingNow == false)
